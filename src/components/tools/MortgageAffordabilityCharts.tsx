            ...
            <ResponsiveContainer width="100%" height="100%" minWidth={280}>
              <ComposedChart margin={{ top: 12, right: 20, left: 6, bottom: 8 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ borderRadius: "12px", borderColor: "#d6d3d1" }}
                  formatter={(value: number, name: string) => {
                    if (name === "Observed Price/Wage") return [`${value.toFixed(2)}x`, name];
                    return [`${value.toFixed(2)}x`, name];
                  }}
                  labelFormatter={(_, payload) => {
                    if (!payload?.length) return "";
                    const p = payload[0].payload as { year?: number; ratePct: number };
                    return p.year ? `Year ${p.year} | Rate ${p.ratePct.toFixed(2)}%` : `Rate ${p.ratePct.toFixed(2)}%`;
                  }}
                />
                <Legend wrapperStyle={{ fontSize: 12 }} />
